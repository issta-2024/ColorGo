        ...
        while [ ${#input_stack[@]} -gt 0 ]; do
            ...
            if [[ "$target " =~ " @@ " ]]; then
                ...
                if [ $exit_code -eq 0 ]; then
                    ...
                    echo "程序执行失败，退出状态码：$exit_code"
                    echo "程序执行失败，退出状态码：$exit_code" >> "$mark_file"
                    ex_time=$(date +%s%3N)
                    echo "EXIT TIME:$(date -d @$ex_time)"
                    echo "EXIT TIME:$(date -d @$ex_time)" >> "$mark_file"
                    total_e_time=$((ex_time - start_time))
                    echo "The total to exit time is: $total_e_time ms"
                    echo "The total to exit time is: $total_e_time ms" >> "$mark_file"
                    crush_flag=1
                    break
