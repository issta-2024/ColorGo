...
void Symbolizer::visitSelectInst(SelectInst &I) {
  ...
  // auto runtimeCall1 = buildRuntimeCall(IRB, runtime.pushPathConstraint,
  //                                      {{I.getCondition(), true},
  //                                       {I.getCondition(), false},
  //                                       {IRB.getInt1(true), false},
  //                                       {getTargetPreferredInt(&I), false}});
  // registerSymbolicComputation(runtimeCall1);

  // auto runtimeCall2 = buildRuntimeCall(IRB, runtime.pushPathConstraint,
  //                                      {{I.getCondition(), true},
  //                                       {I.getCondition(), false},
  //                                       {IRB.getInt1(false), false},
  //                                       {getTargetPreferredInt(&I), false}});
  // registerSymbolicComputation(runtimeCall2);
...
void Symbolizer::setTargetBB(std::unordered_set<llvm::BasicBlock *> t) {
  targetBB = t;
};
void Symbolizer::setSkipTerminate(bool b) { skip = b; };

bool Symbolizer::HasWaytoTargetBB(llvm::BasicBlock *source) {
  if (targetBB.count(source) != 0)
    return true;
  return false;
}
...
void Symbolizer::visitBranchInst(BranchInst &I) {
  ...
  bool directToTure = HasWaytoTargetBB(trueBB);
  bool directToFalse = HasWaytoTargetBB(falseBB);
  if (!directToFalse && !directToTure && !skip) {
    IRBuilder<> builder(&I);
    FunctionCallee exitFunc =
        I.getParent()->getParent()->getParent()->getOrInsertFunction(
            "exit", builder.getInt32Ty(), builder.getInt32Ty());
    Value *exitCode = builder.getInt32(1000);
    builder.CreateCall(exitFunc, exitCode);
  }
  if (directToTure && !directToFalse) {
    IRBuilder<> IRB(&I);
...
void Symbolizer::visitSwitchInst(SwitchInst &I) {
  ...
  if (targetBB.empty())
    return;
  const int caseNum = I.getNumSuccessors();
  std::vector<bool> caseSelect(caseNum);
  bool selectOne = false, selectAll = true;
  for (int i = 0; i < caseNum; i++) {
    caseSelect[i] = false;
    llvm::BasicBlock *selectedBB = I.getSuccessor(i);
    if (HasWaytoTargetBB(selectedBB)) {
      caseSelect[i] = true;
      selectOne = true;
    } else {
      selectAll = false;
  ...
  if (!selectOne && !skip) {
    ...
    FunctionCallee exitFunc =
        I.getParent()->getParent()->getParent()->getOrInsertFunction(
            "exit", builder.getInt32Ty(), builder.getInt32Ty());
    Value *exitCode = builder.getInt32(2000);
    builder.CreateCall(exitFunc, exitCode);
  }
  IRBuilder<> IRB(&I);
  ...
  int i = 1;
  auto *case0Constraint = IRB.CreateCall(runtime.buildTrue);
  Value *case0Taken = IRB.getInt1(true);
  for (auto &caseHandle : I.cases()) {
    auto *caseNoTaken = IRB.CreateICmpNE(condition, caseHandle.getCaseValue());
    auto *caseNotConstraint = IRB.CreateCall(
        runtime.comparisonHandlers[CmpInst::ICMP_NE],
        {conditionExpr, createValueExpression(caseHandle.getCaseValue(), IRB)});
    case0Constraint = IRB.CreateCall(runtime.buildBoolAnd,
                                     {case0Constraint, caseNotConstraint});
    case0Taken = IRB.CreateAnd(case0Taken, caseNoTaken);
    if (caseSelect[i]) {
      ...
      IRB.CreateCall(runtime.pushPathConstraint,
                     {caseConstraint, caseTaken, IRB.getInt1(caseSelect[i]),
  ...
  if (caseSelect[0]) {
    IRB.CreateCall(runtime.pushPathConstraint,
                   {case0Constraint, case0Taken, IRB.getInt1(caseSelect[0]),
                    getTargetPreferredInt(&I)});
  }
}
...
void Symbolizer::tryAlternative(IRBuilder<> &IRB, Value *V) {
  ...
  if (destExpr != nullptr) {
    ...
    auto *pushAssertion =
        IRB.CreateCall(runtime.pushPathConstraint,
                       {destAssertion, IRB.getInt1(true), IRB.getInt1(false),
                        getTargetPreferredInt(V)});
