...
void _sym_push_path_constraint(SymExpr constraint, int taken, int want,
                               uintptr_t site_id) {
  if (constraint == nullptr)
    return;
  fprintf(stderr, "taken=%d, want=%d\n", taken, want);
  fprintf(stderr, "%s\n", _sym_expr_to_string(constraint));
  g_solver->addJcc(allocatedExpressions.at(constraint), taken != 0, want != 0,
                   site_id);
}
