...
void tryAlternative(V value, SymExpr valueExpr, F caller) {
  ...
  if (valueExpr) {
    _sym_push_path_constraint(
        ...
        true, false, reinterpret_cast<uintptr_t>(caller));
  }
}
...
const char *SYM(strchr)(const char *s, int c) {
  ...
  auto *rExpr = _sym_build_equal(
      (*shadowIt != nullptr) ? *shadowIt : _sym_build_integer(s[0], 8), cExpr);
  for (size_t i = 1; i < length; i++) {
    ++shadowIt;
    rExpr = _sym_build_bool_or(
        rExpr,
    ...
    //         cExpr),
    //     /*taken*/ 1, 0, reinterpret_cast<uintptr_t>(SYM(strchr)));
  }
  _sym_set_return_expression(_sym_build_zext(_sym_build_bool_to_bit(rExpr), sizeof(const char *) * 8 - 1));
...
int SYM(memcmp)(const void *a, const void *b, size_t n) {
  ...
  // _sym_push_path_constraint(allEqual, result == 0, result != 0,
